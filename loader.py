...
class PreprocessedImageDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        inp, known_mask, target = get_in_out(image["image"])
        return inp, known_mask, target
...
def crop(image_array: torch.Tensor, border_x: tuple, border_y: tuple):
    ...
    known_array[
    border_x[0]: (known_array.shape[0] - border_x[1]), border_y[0]: (known_array.shape[1] - border_y[1])
    ] = True
...
def get_in_out(image, border_x=None, border_y=None):
    """
...
def train_test_split(
        dataset: Dataset, train_size: float = 0.7482291345857714, val_size: float = 0.12588543270711428, seed: int = 0
):
    """
    ...
    val = torch.utils.data.Subset(
        dataset, indices=np.arange(int(len(dataset) * train_size), int(len(dataset) * (val_size + train_size)))
    )
    test = torch.utils.data.Subset(
        dataset, indices=np.arange(int(len(dataset) * (train_size + val_size)), int(len(dataset)))
    )  # just use remainder of dataset
    return train, val, test


def image_collate_fn(image_batch: list, n_feature_channels: int = 1):
    # Get the maximum number of height and width of image
    max_X = np.max([image[0][0].shape[0] for image in image_batch])
    max_Y = np.max([image[0][0].shape[1] for image in image_batch])
    # Allocate tensors that can fit all images
    stacked_images_input = torch.zeros(size=(len(image_batch), n_feature_channels, max_X, max_Y), dtype=torch.float32)
